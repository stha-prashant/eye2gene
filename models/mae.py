...
class MaskedAutoencoderViT(nn.Module):
    ...
    """
    def __init__(self, configs, norm_layer=nn.LayerNorm, norm_pix_loss=False, device="cuda:0"):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            Block(self.embed_dim, self.num_heads, self.mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
        ...
        self.decoder_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, self.decoder_embed_dim), requires_grad=False)  # fixed sin-cos embedding

        self.decoder_blocks = nn.ModuleList([
            Block(self.decoder_embed_dim, self.decoder_num_heads, self.mlp_ratio, qkv_bias=True, norm_layer=norm_layer)
    ...
    def init_configs(self, configs):
        self.img_size = configs.img_sz
        self.patch_size = configs.patch_sz
        self.in_chans = configs.in_chans
        self.embed_dim = configs.emb_dim
        self.depth = configs.depth
        self.num_heads = configs.num_heads
        self.decoder_embed_dim = configs.dec_emb_dim
        self.decoder_depth = configs.dec_depth
        self.decoder_num_heads = configs.dec_num_heads
    ...
    def initialize_weights(self):
        ...
        pos_embed = get_2d_sincos_pos_embed(self.pos_embed.shape[-1], self.patch_embed.num_patches, cls_token=True)
        self.pos_embed.data.copy_(pos_embed.float())

        decoder_pos_embed = get_2d_sincos_pos_embed(self.decoder_pos_embed.shape[-1], self.patch_embed.num_patches, cls_token=True)
