...
# from utils.builder import build_data, build_datasets
# import wandb
# #########################################################
# ## Input Parameters
# #########################################################
...
# # main
# parser.add_argument("--config", default="config/mae/vae_kaggle_oct_small.yaml", help="config file including parameters for model, data and training")
...
# wandb.init(project=configs.wandb.project, entity=configs.wandb.username, name=configs.exp_name, reinit=True)
# train_dataset, _, _, _ = build_datasets(configs)
# ## Calculate flatten size of each input data
# args.input_size = np.prod(train_dataset[0]["image"].size())
...
wandb.init(project="Eye2Gene", entity="pstha", name="GMVAE_mnist_single", reinit=True)
## Random Seed
SEED = args.seed
np.random.seed(SEED)
random.seed(SEED)
